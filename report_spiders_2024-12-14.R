...
pcoa_viz <- function(x, st = "", xx = 1, yy = 1){ 
    x$pcoa %>% 
    mutate(tur = as.factor(tur),
           Axis.1 = Axis.1 * xx, 
           Axis.2 = Axis.2 * yy,
           ) %>% 
    ggplot(aes(x = Axis.1, y = Axis.2, fill = km, shape = tur)) + 
        geom_point() + 
        scale_shape_manual(values = c(21, 22)) +
        scale_fill_gradient2(
            low = "darkred", mid = "orange", high = "green",
            midpoint = 10) +
        labs(subtitle = st, 
...
ggsave("export/А. Обилия.png", height = 8*0.7, width = 11*.7, dpi = 600)
pcoa_res$raw$permanova <- vegan::adonis2(
    pcoa_res$raw$dis ~ year + tur + km, 
...
ggsave("export/B. Доли.png", height = 8*0.7, width = 11*.7, dpi = 600)
pcoa_res$freq$permanova <- vegan::adonis2(
    pcoa_res$freq$dis ~ year + tur + km, 
...
ggsave("export/C. Состав.png", height = 8*0.7, width = 11*.7, dpi = 600)
pcoa_res$bin$permanova <- vegan::adonis2(
    pcoa_res$bin$dis ~ km + year + tur, 
